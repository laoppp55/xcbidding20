    ...
    <div class="list_main_box" id="datalistid">
        <div id="bcontentid">
            ...
            <div align="left" style="padding-left:30px;">
                <%if (budgetProject.getBuymethod().equals("3")) {%>
                <p align="center"></p>
                <p>
                <p></p>
                ...
                <p><strong>集采机构联系方式：</strong></p>
                <p>集采机构：<%=bulletinNotice.getBuyerName()%></p>
                <p>地址：<%=bulletinNotice.getBuyerAddress()%></p>
                <p>联系方式：<%=bulletinNotice.getBuyerContactor() + "," + bulletinNotice.getBuyerContactPhone()%></p>
                <p><strong>一、采购项目内容及预算金额</strong></p>
                <p>预算金额：<%=String.format("%.4f",bulletinNotice.getNegotiationFileSelling())%> 元(人民币)</p>
                <p>获取资格预审文件的时间：
                    <%=bulletinNotice.getNegotiationFileStartTime()==null?"":sdf.format(bulletinNotice.getNegotiationFileStartTime())%>
                    至
                    <%=bulletinNotice.getNegotiationFileEndTime()==null?"":sdf.format(bulletinNotice.getNegotiationFileEndTime())%>
                    (双休日及法定节假日除外)
                </p>
                <p>获取资格预审文件的地点：<%=bulletinNotice.getNegotiationFileSite()%></p>
                <p>获取资格预审文件的方式：
                <%
                if (bulletinNotice.getNegotiationFileMode().equals("1"))
                    out.println("网上下载");
                else if (bulletinNotice.getNegotiationFileMode().equals("3"))
            ...
            %></p>
                ...
                <p><strong>三、采购项目需要落实的政府采购政策：</strong></p>
                <p><%=bulletinNotice.getProcurementPolicy()%></p>
                <p><strong>四、审查标准、方法：</strong></p>
                <p>详见附件</p>
                <p><strong>五、资格审查日期：</strong></p>
                <p><strong>资格预审申请文件递交截止时间：
                ...
                <p><strong>资格审查地址：</strong>详见附件</p>
                <p><strong>六、采购项目的名称、数量、简要规格描述或项目基本概况介绍：</strong></p>
