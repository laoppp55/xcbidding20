    ...
    <div class="list_main_box" id="datalistid">
        <div id="bcontentid">
            ...
            <div align="left" style="padding-left:30px;">
                ...
                <p>
                <p></p>
                <h3>项目概况</h3> <%=(bulletinConsultationsNotice.getBulletincontent()!=null)?bulletinConsultationsNotice.getBulletincontent():""%>
                ...
                <p><%=(bulletinConsultationsNotice.getQualificationRequirements()!=null)?bulletinConsultationsNotice.getQualificationRequirements():""%></p>
                <p></p>
                ...
                <p>时间：<%=(bulletinConsultationsNotice.getConsultationFileStartTime()==null)?"":sdf.format(bulletinConsultationsNotice.getConsultationFileStartTime()) + "至" + ((bulletinConsultationsNotice.getConsultationFileEndTime()==null)?"":sdf.format(bulletinConsultationsNotice.getConsultationFileEndTime())) + "双休日及法定节假日除外"%></p>
                <p>地点：<%=(bulletinConsultationsNotice.getConsultationFileSite()!=null)?bulletinConsultationsNotice.getConsultationFileSite():""%></p>
                <p>方式：
                    <%
                ...
                <p>售价：￥<%=(bulletinConsultationsNotice.getConsultationFileSelling()!=null)?bulletinConsultationsNotice.getConsultationFileSelling().floatValue():""%> 元</p>
                <p><strong>四、提交投标文件截止时间、开标时间和地点</strong></p>
                <p>开标时间： <%=sdf.format(bulletinConsultationsNotice.getResponseFileStartTime())%>（北京时间）</p>
                <p>开标地点：<%=(bulletinConsultationsNotice.getResponseFileStartSite()!=null)?bulletinConsultationsNotice.getResponseFileStartSite():""%></p>
                <p><strong>五、公告期限</strong></p>
                <p>自本公告发布之日起5个工作日。</p>
                <p><strong>六、其他补充事宜</strong></p>
                <p>
                <p><%=(bulletinConsultationsNotice.getSupplementaryMatters()!=null)?bulletinConsultationsNotice.getSupplementaryMatters():""%></p>
                <!--p>评标方法和标准：综合评分法；本次评审均采用百分制，满分为100分。</p-->
                </p>
                <p><strong>七、对本次招标提出询问，请按以下方式联系。</strong></p>
                <p>1.采购人信息</p>
                <p>名 称：<%=bulletinConsultationsNotice.getBuyerName()%>　　　　　</p>
                <p>地址：<%=bulletinConsultationsNotice.getBuyerAddress()%>　　　　　　　　</p>
                <p>联系方式：<%=bulletinConsultationsNotice.getBuyerContactor() + "," + bulletinConsultationsNotice.getBuyerContactPhone()%>　　　　　　</p>
                <p>2.采购代理机构信息</p>
                <p>名 称：<%=bulletinConsultationsNotice.getAgentName()%>　　　　　　　　　　　　</p>
                <p>地　址：<%=(bulletinConsultationsNotice.getAgentAddress()!=null)?bulletinConsultationsNotice.getAgentAddress():""%>　　　　　　　　　　　　</p>
                <p>联系方式：<%=bulletinConsultationsNotice.getAgentContactor() + "," + bulletinConsultationsNotice.getAgentContactPhone()%>　　　　　　　　　　　　</p>
                <p>3.项目联系方式</p>
                <p>项目联系人：<%=bulletinConsultationsNotice.getProjcontactor()%></p>
                <p>电　话：　　<%=bulletinConsultationsNotice.getProjContactPhone()%></p>
                </p>
                <p>八、采购项目需要落实的政府采购政策：</p>
                <p><%=(bulletinConsultationsNotice.getProcurementPolicy()!=null)?bulletinConsultationsNotice.getProcurementPolicy():""%></p>
                <p></p>
                <p></p>
                <p>
                    <%
                ...
                <p>
                    <%
                        if (bulletinConsultationsNotice.getType().equals("1")) {
                            if (bulletinConsultationsNotice.getNegotiationNoticeURL()!=null) out.println("<a href=" + MyConstants.getDownloadAddress() + "/oa/common/attachment/publicDownloadFile?id=" + bulletinConsultationsNotice.getNegotiationNoticeURL() + "><span style=\"color:red\">其它附件</span></a>");
                        }else {
                            if (bulletinConsultationsNotice.getConsultationAnnouncementURL()!=null) out.println("<a href=" + MyConstants.getDownloadAddress() + "/oa/common/attachment/publicDownloadFile?id=" + bulletinConsultationsNotice.getConsultationAnnouncementURL() + "\"><span style=\"color:red\">公告附件</span></a>");
                        }
                    %>
                </p>
                <p>
