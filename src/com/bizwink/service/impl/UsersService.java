...
public class UsersService implements IUserService {
    ...
    public int createUserAndEnterpriseInfo(Users user,PurchasingAgency suppinfo) {
        ...
        if (errcode==0){
            //保存公司营业执照图片信息
            String filename = suppinfo.getBusinessAttachmentIds();
            if (filename!=null) {
                ...
                baseAttachment.setCreatorId(user.getUSERID());
                baseAttachment.setRootPath(MyConstants.getSftpRootpath());
                String filepath = MyConstants.getSftpRelatePath() + suppinfo.getLegalCode() + File.separator + filename;
                ...
                suppinfo.setBusinessAttachmentIds(uuid);
            }

            //保存公司风险承诺书图片信息
            filename = suppinfo.getCertificateAttachmentIds();
            if (filename!=null) {
                String uuid = UUID.randomUUID().toString();
                uuid = uuid.replace("-", "");
                BaseAttachment baseAttachment = new BaseAttachment();
                ...
                baseAttachment.setFilepath(filepath);
                File thefile = new File(filepath);
                baseAttachment.setFileSize((double) thefile.length()/(1024*1024));     //转化为M
                errcode = baseAttachmentMapper.insert(baseAttachment);
                suppinfo.setCertificateAttachmentIds(uuid);
            }
    ...
    public Users getUserByCompcode(String compcode,int usertype) {
        Map params = new HashMap();
        params.put("compcode",compcode);
        params.put("usertype",usertype);

        return usersMapper.getUserByCompcodeAndUserType(params);
    }
    ...
    public List<Users> getUsersByCustomer(BigDecimal customer) {
        return usersMapper.getUsersByCustomer(customer);
    }

    public int updateUserinfoByUseridSelective(Users user) {
        return usersMapper.updateByUseridSelective(user);
    }
    ...
    public PurchasingAgency getEnterpriseInfo(String compname) {
        return purchasingAgencyMapper.selectEnterpriseInfoByCompname(compname);
    }

    public PurchasingAgency getEnterpriseInfoByCompcode(String compcode) {
        return purchasingAgencyMapper.selectEnterpriseInfoByCompcode(compcode);
    }

    public int updateEnterpriseInfo(PurchasingAgency purchasingAgency,String userid) {
        int errcode = 0;
        //保存公司营业执照图片信息
        ...
        return purchasingAgencyMapper.updateByPrimaryKeyWithBLOBs(purchasingAgency);
    }
