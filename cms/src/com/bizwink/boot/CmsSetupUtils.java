...
//add by hu
      m_cfgFolder = basePath + "WEB-INF/classes/com/bizwink/cms/server/";
    ...
    public static String escape(String source) {
        ...
        try{
            String enc = URLEncoder.encode(source,"iso-8859-1");
     ...
     *  @param backup if true, create backupfile
     */
    public void saveCfgProperties(ExtendedProperties extProp, String originalFile, boolean backup)  {
        if (new File(m_cfgFolder + originalFile).isFile()) {
          //  String backupFile = originalFile.substring(0,originalFile.lastIndexOf('.')) + ".ori";
            String tempFile = originalFile.substring(0,originalFile.lastIndexOf('.')) + ".tmp";
            ...
            // make a backup copy
            //      if(backup)  {
            //          backup1(originalFile, backupFile);
            //      }

            //save to temporary file
            backup1(originalFile, tempFile);

            // save properties
            save1(extProp, tempFile, originalFile);

            // delete temp file
            File temp = new File(m_cfgFolder + tempFile);
            temp.delete();
        }
        else  {
            m_errors.addElement("No valid file: " + originalFile+ "\n");
     ...
     *  @param backupFile target file
     */
    private void backup1(String originalFile, String backupFile) {
        try {
            LineNumberReader lnr = new LineNumberReader(new FileReader(new File(m_cfgFolder
                    + originalFile)));
            FileWriter fw = new FileWriter(new File(m_cfgFolder + backupFile));
            while (true)  {
                String line = lnr.readLine();
                if(line == null) break;
                fw.write(line+'\n');
     ...
     * @param target target file to save the properties to
     */
    private void save1(ExtendedProperties extProp, String source, String target) {
        try {
            LineNumberReader lnr = new LineNumberReader(new FileReader(new File(m_cfgFolder
                    + source)));

            FileWriter fw = new FileWriter(new File(m_cfgFolder + target));
