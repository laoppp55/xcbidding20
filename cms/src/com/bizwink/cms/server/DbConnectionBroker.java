...
public class DbConnectionBroker implements Runnable {
    ...
    private String language, type, dbDriver, dbServer, dbLogin, dbPassword, logFileString;
     ...
     */
    public DbConnectionBroker(String language, String type, String dbDriver, String dbServer, String dbLogin,
                              String dbPassword, int minConns, int maxConns,
                              String logFileString, double maxConnTime) throws IOException {

        setupBroker(language, type, dbDriver, dbServer, dbLogin, dbPassword, minConns,maxConns, logFileString, maxConnTime, true,DEFAULTMAXCHECKOUTSECONDS, DEFAULTDEBUGLEVEL);
    }
    ...
    public DbConnectionBroker(String language, String type, String dbDriver, String dbServer, String dbLogin,
        ...
        setupBroker(language, type, dbDriver, dbServer, dbLogin, dbPassword, minConns,
    ...
    private void setupBroker(String language, String type, String dbDriver, String dbServer, String dbLogin,
                             String dbPassword, int minConns, int maxConns,
                             String logFileString, double maxConnTime, boolean logAppend,
                             int maxCheckoutSeconds, int debugLevel)
            throws IOException {
        ...
        this.type = type;
        maxConnMSec = (int) (maxConnTime * 60000);   //设置连接最大的分钟数
        if (maxConnMSec < 30000) {                   //最少每半分钟需要检查连接是否可用
            maxConnMSec = 30000;
        }
        ...
        System.out.println("debugdelvel==" + debugLevel);
    ...
    public void run() {
        ...
        long maxCheckoutMillis = maxCheckoutSeconds * 60000;          //超出设定时间的连接需要断开连接，重新建立连接
        ...
        if (available) {
            ...
            while (!gotOne) {
                ...
                try {
                    ...
                    //if (gotOne)
                    //    log.println("get a connection ==" + idOfConnection(conn) + " --> " + (new SimpleDateFormat("MM/dd/yyyy  hh:mm:ss a")).format(new java.util.Date()));
                }
                catch (SQLException e1) {
                    log.println("Error: " + e1.getMessage());
                }

                if (gotOne) {
                    //log.println("get new connection ==" + idOfConnection(conn) + " --> " + (new SimpleDateFormat("MM/dd/yyyy  hh:mm:ss a")).format(new java.util.Date()));
                    break;
                } else {
                    synchronized (this) {  // Add new connections to the pool
                        if (currConnections < maxConns) {
                            try {
                                createConn(currConnections, type);
                                currConnections++;
                                //log.println("create new connection: " + currConnections);
                            } catch (SQLException e) {
                                if (debugLevel > 0) {
                    ...
                    if (debugLevel > 1) {
                        log.println("-----> Connections Exhausted!  Will wait and try again in loop " + String.valueOf(outerloop));
                    }
                }
    ...
    private void createConn(int i, String type) throws SQLException {
        Date now = new Date();
        if (type.equals("oracle")) {    //for oracle connection pool
            try {
            ...
            //for weblogic connection pool
            try {
                Class.forName(dbDriver);
                java.util.Properties prop = new java.util.Properties();
                prop.put("user", dbLogin);
            ...
            //for jdbc-odbc connection pool
            try {
                Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
                String strurl = "jdbc:odbc:driver={Microsoft Access Driver (*.mdb)};DBQ=e:\\webbuilder\\webbuilder.mdb";
                java.util.Properties prop = new java.util.Properties();
                prop.put("user", dbLogin);
                prop.put("password", dbPassword);
                connPool[i] = DriverManager.getConnection(strurl);
                //connPool[i] = DriverManager.getConnection(dbServer,dbLogin,dbPassword);
                connStatus[i] = 0;
                connID[i] = connPool[i].toString();
                connLockTime[i] = 0;
            ...
            try {
                ...
                //String url =dbServer + "?user=" + dbLogin + "&password=" + dbPassword + "&useUnicode=true&characterEncoding=8859_1";
                connPool[i]= DriverManager.getConnection(dbServer, dbLogin, dbPassword);
                connStatus[i] = 0;
                connID[i] = connPool[i].toString();
                connLockTime[i] = 0;
