...
Dim ArgIPAddress, ArgRootDirectory, ArgServerComment, ArgSkeletalDir, ArgHostName, ArgPort, ArgDefaultDoc
Dim CMSRoot
...
ArgIPAddress = ""
ArgHostName = ""
ArgDefaultDoc = "index.htm,index.jsp,index.shtml,index.shtm"
ArgPort = 80
ArgStart = True
...
While ArgNum < oArgs.Count
	...
	Select Case LCase(oArgs(ArgNum))
		...
		Case "--defaultdoc","-d":
		        ArgNum = ArgNum + 1
		        ArgDefaultDoc = oArgs(ArgNum)
		Case "--cmsroot","-w":
		        ArgNum = ArgNum + 1
		        CMSRoot = oArgs(ArgNum)
                        WScript.Echo CMSRoot		        
		Case "--dontstart":
			ArgStart = False
		Case "--help","-?":
			Call DisplayUsage
...
Call ASTCreateWebSite(ArgIPAddress, ArgRootDirectory, ArgServerComment, ArgHostName, ArgPort, ArgComputers, ArgStart, ArgDefaultDoc)

Sub ASTCreateWebSite(IPAddress, RootDirectory, ServerComment, HostName, PortNum, Computers, Start, defaultdoc)
	...
	For ComputerIndex = 0 To UBound(Computers)
		...
		'Set the scripts
		Set NewDir = NewWebServer.Create("IIsWebVirtualDir", "ROOT/scripts")
		NewDir.Path = "c:\InetPub\scripts"
		NewDir.AccessRead = True
		NewDir.ContentIndexed = True
		NewDir.AccessExecute = True
		NewDir.EnableDirBrowsing = False
		Err.Clear
		NewDir.SetInfo
		If (Err.Number = 0) Then
			Trace "scripts virtual directory set."
		Else
			Display "Error setting scripts virtual directory."
		End If

		'Set the _sys_js virtual directory
		Set NewDir = NewWebServer.Create("IIsWebVirtualDir", "ROOT/_sys_js")
		NewDir.Path = CMSRoot + "sites\_commons\js"
		NewDir.AccessRead = True
		NewDir.ContentIndexed = True
		NewDir.AccessExecute = True
		NewDir.EnableDirBrowsing = False
		...
		'Set the _sys_css virtual directory
		Set NewDir = NewWebServer.Create("IIsWebVirtualDir", "ROOT/_sys_css")
		NewDir.Path = CMSRoot + "sites\css"
		NewDir.AccessRead = True
		NewDir.ContentIndexed = True
		NewDir.AccessExecute = True
		NewDir.EnableDirBrowsing = False
		...
		'Set the _sys_images virtual directory
		Set NewDir = NewWebServer.Create("IIsWebVirtualDir", "ROOT/_sys_images")
		NewDir.Path = CMSRoot + "sites\images"
		NewDir.AccessRead = True
		NewDir.ContentIndexed = True
		NewDir.AccessExecute = True
		NewDir.EnableDirBrowsing = False
		...
		'Set the _sys_images virtual directory
		Set NewDir = NewWebServer.Create("IIsWebVirtualDir", "ROOT/fckeditor")
		NewDir.Path = CMSRoot + "fckeditor"
		NewDir.AccessRead = True
		NewDir.ContentIndexed = True
		NewDir.AccessExecute = True
		NewDir.EnableDirBrowsing = False
		...
		'Set the webbuilder virtual directory
		Set NewDir = NewWebServer.Create("IIsWebVirtualDir", "ROOT/webbuilder")
		NewDir.Path = CMSRoot
		NewDir.AccessRead = True
		NewDir.ContentIndexed = True
		NewDir.AccessExecute = True
