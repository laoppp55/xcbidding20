...
<%
    ...
    int current_siteid = authToken.getSiteID();
    ...
    <script language="Javascript">
        ...
        function select_the_template(form) {
            ...
            if (value==0) {
                ...
                //alert(value);
                form.action="../member/makeSiteFromSampleSite.jsp?samsiteid=" + value;
                form.method="post";
                //form.submit();
            }
        }
    ...
    <table width="178" border="0" cellpadding="0" cellspacing="0" background="images/product-index_17.gif">
        ...
        <tr>
            <td height="34" align="left" valign="middle" background="images/product-index_02.gif">
                <ul>
                    <li class="li_product"><%=(sitetype==0)?"企业网站":"<a href=\"weblist.jsp?type=0\">企业网站</a>"%></li>
                </ul>
            </td>
        </tr>
        <tr>
            <td height="34" align="left" valign="middle" background="images/product-index_02.gif">
                <ul>
                    <li class="li_product"><%=(sitetype==1)?"电子商务":"<a href=\"weblist.jsp?type=1\">电子商务</a>"%></li>
                </ul>
            </td>
        </tr>
        <tr>
            <td height="34" align="left" valign="middle" background="images/product-index_02.gif">
                <ul>
                    <li class="li_product"><%=(sitetype==2)?"个人网站":"<a href=\"weblist.jsp?type=2\">个人网站</a>"%></li>
                </ul>
            </td>
        </tr>
        <tr>
...
<form name="selmodel" onsubmit="return select_the_template(selmodel);">
    <tr>
        ...
        <td align="left" valign="top" background="images/product-index_20.gif">
            ...
            <table width="761" border="0" cellspacing="0" cellpadding="0">
                ...
                <td width="160" align="center" valign="top"><a href="javascript:openwin('<%=domainname%>')"><img src="../sitespic/<%=(sitepic!=null)?sitepic:"product_331.jpg"%>" width="160" height="123" border="0" /></a><br />
                    <br />
                    <input type="radio" name="site_radio" value=<%=siteid%> />NO.<%=siteid%>
            ...
            <TR>
                <TD>总共<%=totalpages%>页&nbsp;&nbsp; 共<%=totals%>条&nbsp;&nbsp; 当前第<%=currentpage%>页&nbsp;
                    <%
                        if(startnum>0){
                    %>
                    <a href="weblist.jsp?type=<%=sitetype%>&startnum=0&searchflag=<%=searchflag%>&search=<%=search%>">第一页</a>
                    <%}%>
                    <%if((startnum-resultnum)>=0){%>
                    <a href="weblist.jsp?type=<%=sitetype%>&startnum=<%=startnum-resultnum%>&searchflag=<%=searchflag%>&search=<%=search%>">上一页</a>
                    <%}%>
                    <%if((startnum+resultnum)<totals){%>
                    <A href="weblist.jsp?type=<%=sitetype%>&startnum=<%=startnum+resultnum%>&searchflag=<%=searchflag%>&search=<%=search%>">下一页</A>
                    <%}%>
                    <%if(currentpage != totalpages){%>
                    <A href="weblist.jsp?type=<%=sitetype%>&startnum=<%=(totalpages-1)*resultnum%>&searchflag=<%=searchflag%>&search=<%=search%>">最后一页</A>
                    <%}%>
                </TD>
                <TD>&nbsp;</TD>
            </TR></TBODY></TABLE><br>
