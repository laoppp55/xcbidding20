...
<%
  ...
  int globalSiteID = regMgr.getGlobalSite();
  Tree colTree = TreeManager.getInstance().getSiteTreeIncludeGlobalColumn(siteid,globalSiteID);
    ...
    for(i=0;i<colTree.getNodeNum();i++)
    {
      if(treeNodes[i].getLinkPointer() == currentID)
      {
        if (treeNodes[i].getGlobalColumn() == 1) {
          String name  = "<font class=line><font color='red'>"+StringUtil.gb2iso4View(treeNodes[i].getChName())+"</font></font>";
          buf.append(menuName).append(".MTMAddItem(new MTMenuItem(\"");
          buf.append(name).append("\",\"addArtReferRight.jsp?column="+treeNodes[i].getId()+"\""+", \"cmsright\"));");
          buf.append("\n");
        } else {
