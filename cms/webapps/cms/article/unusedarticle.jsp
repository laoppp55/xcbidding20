...
<%
    ...
    List articleList = articleMgr.getUnusedArticles(columnID, start, range);
    int total = articleMgr.getUnusedArticlesNum(columnID);
...
<head>
    ...
    <script language="javascript">
        ...
        function Update(articleID, emptycontentflag)
        {
        <%if (isDefineAttr == 2){%>
        ...
        <%}else{%>
            if(emptycontentflag == 0)
                window.open("editarticle.jsp?article=" + articleID + "&range=<%=range%>&start=<%=start%>&fromflag=u", "Update", "width=930,height=650,left=5,top=5,status");
            else
                window.location="../upload/edituploadfile.jsp?article=" + articleID + "&range=<%=range%>&start=<%=start%>&fromflag=u";
        <%}%>
        }
    </script>
</head>
...
<table border=1 borderColorDark=#ffffec borderColorLight=#5e5e00 cellPadding=0 cellSpacing=0 width='100%'>
    ...
    <tr bgcolor="<%=bgcolor%>" height=25>
        <td align=center><img src="../images/button/wei.gif" alt="未使用文章"></td>
        <td><%= maintitle %>
        </td>
        ...
        <td align=center><a href=javascript:Preview(previewarticle.jsp?article=<%=articleID%>);><img
                src="../images/button/view.gif" border=0></a></td>
        <td align=center><a href=javascript:Update(<%=articleID%>,<%=emptycontentflag%>);><img src="../images/button/edit.gif" align="bottom"
                                                                         border=0></a></td>
        <td align=center><a
                href="removearticle.jsp?article=<%=articleID%>&range=<%=range%>&start=<%=start%>&fromflag=u"><img
                src="../images/button/del.gif" align="bottom" border=0></a></td>
    </tr>
    <%}%>
</table>
</BODY>
