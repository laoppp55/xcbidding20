...
<%
    ...
    do
    {
        ...
        for(i=0;i<colTree.getNodeNum();i++)
        {
            if(treeNodes[i].getLinkPointer() == currentID)
            {
                ...
                //buf.append(name).append("\",\"addArticleAttechmentRight.jsp?column="+treeNodes[i].getId()+"\", \"cmsright\"));");
                buf.append(name).append("\",\"javascript:selectItem(" + treeNodes[i].getId() + ",'" + treeNodes[i].getChName() + "'); \",\"cmsleft\"));");
                buf.append("\n");
            }
        }
    ...
    FileUtil.writeFile(buf,"c:\\1111.txt");
%>
<html>
<head>
    <title></title>
