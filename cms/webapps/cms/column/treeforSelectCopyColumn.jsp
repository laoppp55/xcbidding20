...
<%
    ...
    int f = ParamUtil.getIntParameter(request, "f", -1);
%>
<html>
<head>
    <title></title>
    ...
    do {
        ...
        for (i = 0; i < colTree.getNodeNum(); i++) {
            if (treeNodes[i].getLinkPointer() == currentID) {
                ...
                buf.append(name).append("\",\"javascript:selectCopyColumn(" + treeNodes[i].getId() + ",'"+StringUtil.gb2iso4View(treeNodes[i].getChName())+"');\", \"cmsleft\"));");
                buf.append("\n");
            }
        }
...
<script type="text/javascript">
    <%
        if(f == 0){
    %>
    function selectCopyColumn(cid,cname) {
        cname = cname.replace(new RegExp("#", "gm"), " ");
        cname = cname.replace(new RegExp("`", "gm"), "'");
        window.parent.opener.document.getElementById("orgcolumnname").value = cname;
        window.parent.opener.document.getElementById("orgcolumnid").value = cid;
        window.parent.opener.document.getElementById("orgcolumnname").focus();
        top.close();
    }
    <%} else {%>
    function selectCopyColumn(cid,cname) {
        cname = cname.replace(new RegExp("#", "gm"), " ");
        cname = cname.replace(new RegExp("`", "gm"), "'");
        window.parent.opener.document.getElementById("targetcolumnname").value = cname;
        window.parent.opener.document.getElementById("targetcolumnid").value = cid;
        window.parent.opener.document.getElementById("targetcolumnname").focus();
        top.close();
    }
    <%}%>
</script>