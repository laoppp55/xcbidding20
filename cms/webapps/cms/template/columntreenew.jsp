...
<%
    ...
    if (colTree.getNodeNum() > 1) {
        ...
        List<node> pid = new ArrayList<>();                        //遍历树所需要的节点数组，存储当前未处理的节点
        int nodenum = 1;                                              //当前被处理节点的初始值
        int subnodenum = 0;                                              //记录当前节点的子节点数量
...
//            System.out.println(currentNode.getChName() + "===" + currentNode.getId() + "===" + currentNode.getLinkPointer());
            ...
            if(subnodenum==0) {
                if (SubNodeNumOfParentNode > 1) {
                    ...
                    buf.append("      id:" + currentNode.getId() + "\r\n");
                    buf.append("   }\r\n");
                    //判断是否处理整个栏目树的最后一个节点，最后一个节点最后不需要有逗号
                    if(nodenum > 0)
                        buf.append("]\r\n},\r\n");
                    else
                        buf.append("]\r\n}\r\n");
