<%@page import="java.io.*,
                java.util.*,
                java.sql.*,
                java.text.*,
                com.bizwink.cms.util.*,
                com.bizwink.cms.server.*,
                com.bucid.bbs.*" contentType="text/html;charset=gbk"
%>
<%
  int threadid       = ParamUtil.getIntParameter(request, "threadid", 0);
  int forumid        = ParamUtil.getIntParameter(request, "forumid", 0);

  IBBSManager bbsMgr = BBSPeer.getInstance();
  int hitnum = bbsMgr.getNUM("hitnum",threadid) + 1;
  int answernum = bbsMgr.getNUM("answernum",threadid);
  bbsMgr.updateAnswerHitNum(hitnum,answernum,threadid);
  response.sendRedirect("thread.jsp?threadid="+threadid+"&forumid="+forumid);
%>

