...
var MTMLinkedJSURL, MTMLinkedSS = true, MTMSSHREF = "../style/global.css", MTMLinkedInitFunction, MTMDOCTYPE, MTMcontentType, MTMHeader, MTMFooter, MTMrightClickMessage, MTMDefaultTarget = "cmsright", MTMTimeOut = 1;
...
var MTMCodeFrame = "cmsleft", MTMenuFrame = "menu", MTMTableWidth = "100%", MTMenuImageDirectory = "../menu-images/";
var MTMUseToolTips = true, MTMEmulateWE = true, MTMAlwaysLinkIfWE = true, MTMSubsGetPlus = "Submenu", MTMSubsAutoClose = true;
var MTMBackground = "", MTMBGColor = "#dddddd", MTMTextColor = "#000000", MTMLinkColor = "#0000ff", MTMTrackColor = "#000000", MTMAhoverColor = "#000000", MTMSubExpandColor = "#666699", MTMSubClosedColor = "#666699", MTMSubTextColor = "#000000";
var MTMenuText = "<font class=line>网站栏目</font>", MTMRootIcon = "menu_new_root.gif", MTMRootColor = "#000000";
var MTMRootFont = "Arial";
...
function MTMSubAction(SubItem) {
    ...
    if (SubItem.expanded) {
        var abc = SubItem.parentNode;
        var parentNum = 1;
        if (abc != null) parentNum = abc["number"];
        if (parentNum == 1) {                              //点击第一层的下面层，清空保存信息的隐含域，从新设置信息
            document.getElementById("expnameid").value = "";
            document.getElementById("expnameid").value = document.getElementById("expnameid").value + SubItem.number+ ":";
        } else {
            document.getElementById("expnameid").value = document.getElementById("expnameid").value + SubItem.number+ ":";
        }
        MTMExpansion = true;
...
function MTMStartMenu(thisEvent,expmenuids) {
    ...
    if (MTMFirstRun) {
        if (MTMCurrentTime++ == MTMTimeOut) { // notice the post-increment
            setTimeout("MTMDisplayMenu(" + expmenuids + ")", 1);
        } else {
...
function MTMDisplayMenu(expmenuids) {
...
function MTMDisplayItem(item, last) {
    ...
    if (item.submenu) {
        ...
        var MTMouseOverCmd = "parent.status='" + (item.expanded ? "合拢" : "展开") + "';return true;";
...
function MTMakeLink(thisItem, voidURL, addName, addTitle, clickEvent, mouseOverEvent, mouseOutEvent) {
    ...
    if (!voidURL && needDrag) {
        columnID = url.substring(url.indexOf("|") + 1, url.lastIndexOf("|"));
        columnName = url.substring(url.lastIndexOf("|") + 1);
        columnName = columnName.replace("&nbsp;", " ");
        if (url.indexOf("oncontextmenu") != -1) {
            url = url.substring(0, url.indexOf("oncontextmenu"));
        }
        url = url.substring(0, url.indexOf("|")) + "?column=" + columnID;
    }

    if (url.indexOf("oncontextmenu") != -1) {
        rightC = url.substring(url.indexOf("oncontextmenu")+14);
        url = url.substring(0, url.indexOf("oncontextmenu")-1);
    }

    var tempString = '<a href="' + (voidURL ? 'javascript:;' : MTMUA.resolveURL(url)) + '" ';
    if (MTMUseToolTips && addTitle && thisItem.tooltip) {
        tempString += 'title="' + thisItem.tooltip + '" ';
    }
    if((rightC != null)&&(rightC != "")){
        tempString += 'oncontextmenu=' + rightC + ' ';
        //tempString += 'oncontextmenu=javascript:alert(\"test\"); ';
    }
    if (addName) {
        tempString += 'name="sub' + thisItem.number + '" ';
    }
    if (!voidURL && needDrag) {   //jermy for drag
        tempString += "ondrag=window.parent.frames['cmsleft'].Drag(" + columnID + ",'" + columnName + "'); ";
    }
    if (clickEvent) {
    ...
    if (thisItem.submenu && MTMClickedItem && thisItem.number == MTMClickedItem.number) {
        ...
        if (MTMTrackedCookieName) {
            MTMTCArray = new Array(thisItem.number, thisItem.target, url);
        }
