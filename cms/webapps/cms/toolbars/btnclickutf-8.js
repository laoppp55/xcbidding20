...
function MarkName_Add(columnID,sitename)
{
    ...
    alert(isMSIE + "===" + navigator.appName);
    ...
    //---------------------
        ...
        if (isMSIE) {
            ...
            win = window.open(winStr, "视频播放", "font-family=Verdana,font-size=12,width=800px,height=550px,status=no");
            win.focus();
        }
    }
    ...
    else if (mark_value == 'ARTICLE_TYPE')
    {
        ...
        if (isMSIE) {
            ...
            win = window.open(winStr, "登录表单", "font-family=Verdana,font-size=12,width=1200px,height=600px,status=no");
            win.focus();
        }
        ...
        if (isMSIE) {
            ...
            win = window.open(winStr, "用户登录显示", "font-family=Verdana,font-size=12,width=1200px,height=600px,status=no");
            win.focus();
        }
    }
        ...
        if (mname_in_thepage == "[" + marksname + "]")
            ...
            if (isMSIE) {
                ...
                win = window.open(winStr, "地图标注", "font-family=Verdana,font-size=12,width=560px,height=180px,status=no");
                win.focus();
            }
        } else {
            alert("本页已经存在程序标记" + mname_in_thepage + "，请先删除已经存在的程序标记，然后在添加新的程序标记");
