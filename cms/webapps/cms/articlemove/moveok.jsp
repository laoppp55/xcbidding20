...
<%
    ...
    if (execute == 0) {
        ...
        if (!isSame) {
            response.sendRedirect("listarticle.jsp?column=" + oldcolumnID + "&msg=" + StringUtil.gb2iso("两个栏目扩展属性不相同！移动不成功！"));
            return;
        }
    }
    ...
    if (moveall == 0) {
        ...
        try {
            ...
            System.out.println("获得栏目下所有文章的ID出错，在/articlemove/moveok.jsp页面");
            e.printStackTrace();
        }
    }

    try {
        articleMgr.moveArticlesToColumn(oldcolumnID, columnID, articleids, siteid, appPath, username);
    }
    catch (ArticleException e) {
        e.printStackTrace();
    }
