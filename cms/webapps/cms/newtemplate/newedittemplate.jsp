...
<%
    ...
    if (success) {
        response.sendRedirect(response.encodeRedirectURL("newclosewin.jsp?id=" + ID + "&column=" + columnID + "&rightid=" + rightid));
        return;
    }
...
function checkoption(id)
{
    ...
    optionsname[0][24] = "网站版权";
    optionsname[0][25] = "加入包含文件";
    optionsname[0][26] = "最近浏览";
    optionsname[0][27] = "选择已有标记";
...
<form action="newedittemplate.jsp" method="post" name="createForm">
    ...
    <div id=div2 class="tbScriptlet"><textarea name=pubcode></textarea></div >
    <table width="100%" border=0 cellspacing=0 cellpadding=0>
        ...
        <tr height=30>
            ...
            <td width="30%" align=right>
                ...
                <input class=tine type=button value="  取消  " onclick="window.location='newclosewin.jsp?id=<%=ID%>&column=<%=columnID%>&rightid=<%=rightid%>';">&nbsp;&nbsp;
            </td>
        </tr>
        <tr bgcolor=#003366>
            <td colspan=3 height=2></td>
        ...
        <tr>
            <td>
                ...
                <script type="text/javascript">
                    ...
                    {
                        document.createForm.pubcode.value = htmlcode;
                        winStr = "editBizMark.jsp?column=<%=columnID%>&template=<%=ID%>&modeltype=<%=modelType%>&right=<%=rightid%>&siteid=<%=siteid%>&sitename=<%=sitename%>&userid=<%=username%>&rp=<%=rootPath%>&imgflag=<%=imgflag%>";
                        returnvalue = window.open(winStr,"公用代码","width=800px,height=600px,status=yes,scrollbars=yes");
                    }

                    function setHtmlcode(htmlcode)
                    {
                        wmp.SetHtmlcode(htmlcode);
                    }
                </script>
