...
<script language="javascript">
  function uploadpic(bookid){
    window.open("../upload/uploadfiles.jsp?bookid="+bookid+"&comflag=1","图片上传","width=360,height=380,toolbar=no,top=200,left=200,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no");
}

function golistnew(r){
  tehuiForm.action = "add.jsp?startrow="+r;
  tehuiForm.submit();
}
...
function gotoSearch(r,p){
  ...
  SearchForm.action = "add.jsp?searchflag=1";
  SearchForm.submit();
...
function jumppage(r,str){
  tehuiForm.action = "add.jsp?startrow="+r+str;
  tehuiForm.submit();
}
</script>
</head>
            ...
            <td>
              <table width="100%" border="0" cellpadding="2" cellspacing="1">
                ...
                for(int i=0;i<currentlist.size();i++){
                  ...
                  //获得竞标状态
                  int bidflag = bidMgr.getBidFlag(bookid);
                  String bidstr = "";
                  if(bidflag == 0){
                    bidstr = "竞标中";
                  }else
                ...
                %>
                <tr  bgcolor="#FFFFFF">
                  <td align="center"><input type="radio" value="<%=bookid%>" onclick="javascript:uploadpic(<%=bookid%>);">
                  <td align="center"><%=bidstr%></td>
                  <td align="center"><%=booktype%></td>
                  <td align="center"><a href="addbid.jsp?bookid=<%=bookid%>"><%=bookname%></a></td>
                  <td align="center"><%=desc%></td>
                  <td align="center"><%=author%></td>
                  <td align="center"><%=publisher%></td>
                  <td align="center"><%=publishtime==null?"":String.valueOf(publishtime).substring(0, String.valueOf(publishtime).indexOf(" "))%></td>
                  <td align="center"><%=inprice%></td>
                  <td align="center"><%=createdate==null?"":String.valueOf(createdate).substring(0,19)%></td>
                </tr>
                <%}%>
              </table>
            </td>
...
<%
  if(searchflag == 0){
    if((startrow-range)>=0){
%>
[<a href="add.jsp?startrow=<%=startrow-range%>">上一页</a>]
<%}%>
<%
  if((startrow+range)<rows){
%>
[<a href="add.jsp?startrow=<%=startrow+range%>">下一页</a>]
<%}
 if(totalpages>1){%>
...
<%}
    ...
    if((startrow-range)>=0){
%>
  [<a href="add.jsp?startrow=<%=startrow-range%><%=jumpstr%>">上一页</a>]
  <%}
    if((startrow+range)<rows){
  %>
  [<a href="add.jsp?startrow=<%=startrow+range%><%=jumpstr%>">下一页</a>]
  <%}
 if(totalpages>1){
