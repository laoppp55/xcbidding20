...
<%
  ...
  list = bidMgr.listBidBooks(0,0,"1");
  currentlist = bidMgr.listBidBooks(startrow,range,"1");
  ...
  if(searchflag==1){
    ...
    String sqlstr = "select * from tbl_bid where bidflag=1 ";
...
function gotoSearch(r){
  SearchForm.action = "manager.jsp?searchflag=1";
  SearchForm.submit();
}

function jumppage(r,str){
  window.location = "manager.jsp?startrow="+r+"&searchflag=1"+str;
  }
</script>
</head>
      ...
      <td>
        <table width="100%" border="0" cellpadding="0">
          <tr bgcolor="#F4F4F4" align="center">
            <td class="moduleTitle"><font color="#48758C">竞标管理</font></td>
          </tr>
          <tr bgcolor="#d4d4d4" align="right">
            <td>
              <table width="100%" border="0" cellpadding="2" cellspacing="1">
                ...
                %>
                <tr  bgcolor="#FFFFFF">
                  <td align="center" class="txt"><input type="radio" name="end" value="<%=bid.getID()%>"></td>
...
<tr valign="bottom">
<td>
共有<%=rows%>项已结束竞标&nbsp;&nbsp;总<%=totalpages%>页 第<%=currentpage%>页
</td>
<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
<td>
<%
  if((startrow-range)>=0){
%>
[<a href="manager.jsp?startrow=<%=startrow-range%>&searchflag=1<%=jumpstr%>">上一页</a>]
<%}%>
<%
  if((startrow+range)<rows){
%>
[<a href="manager.jsp?startrow=<%=startrow+range%>&searchflag=1<%=jumpstr%>">下一页</a>]
<%}
  if(totalpages>1){
  %>
 ...
 <%}%>
  ...
  <input type="submit" value="存为历史纪录" onclick="javascript:return check(this.form);">
</form>
