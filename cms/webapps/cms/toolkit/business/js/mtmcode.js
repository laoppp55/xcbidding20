...
function MTMDisplayMenu() {
  if(MTMBrowser && !MTMUpdating) {
    ...
    MTMOutputString += 'bgcolor="' + MTMBGColor + '" text="' + MTMTextColor + '" link="' + MTMLinkColor + '" vlink=#cc0000 alink="' + MTMLinkColor + '">';
...
function MTMDisplayItem(item, last) {
  ...
  if(item.submenu) {
    ...
    var MTMfrm = "parent.frames['cmsleft']";
...
function MTMakeVoid(thisItem, thisCmd, thisText) {
  var tempString = "";
  tempString +=  '<a name="sub' + thisItem.number + '" href="javascript:parent.frames[\'cmsleft\'].myVoid();" onclick="' + thisCmd + '" onmouseover="window.status=\'' + thisText + '\';return true;" onmouseout="window.status=\'' + window.defaultStatus + '\';return true;"';
...
function MTMakeLink(thisItem, addName) {
  ...
  if (temp.indexOf("&&") !=-1){
  	regexp1 = /[ ]/g;
        temp = temp.replace(regexp1,'');
        temp = temp.replace("&&"," ");
  	tempString += ' href="#"' + temp;
  }else{
       	if (typetree == 1)
  	    tempString += ' ondrag=parent.frames[\'cmsright\'].dragfile(\''+temp+'\') href="' + MTMCheckURL(temp) + '"';
  	else
  	    tempString += ' href="' + MTMCheckURL(temp) + '"';
  }
  if(thisItem.target != "") {
