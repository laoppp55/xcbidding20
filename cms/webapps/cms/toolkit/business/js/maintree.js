...
function MTMDisplayMenu() {
  if (MTMUA.MTMable && !MTMUpdating) {
    ...
    if (MTMUA.DOMable) {
      ...
      MTMOutputString += '<script language="javascript" src="../js/maintree.js"></script>';
      MTMOutputString += '<script language="javascript" src="../js/mytree.js"></script>';
      if (MTMcontentType) {
      ...
      MTMOutputString += '</head>\n<body onselectstart="return false;" oncontextmenu="return false;" id=treeRange ';
      if (MTMBackground != "") {
...
function MTMakeLink(thisItem, voidURL, addName, addTitle, clickEvent, mouseOverEvent, mouseOutEvent) {
  ...
  var tempString = '<a href="' + (voidURL ? '#' : MTMUA.resolveURL(url)) + '" ';
  ...
  if (!voidURL && needDrag) {   //jermy for drag
    tempString += "ondrag=parent.frames['cmsleft'].Drag(" + columnID + ",'" + columnName + "'); ";
  }
  if (clickEvent) {
