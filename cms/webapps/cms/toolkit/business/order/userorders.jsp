...
<%
    ...
    String showname = ParamUtil.getParameter(request, "showname");
    ...
    sqlstr = "select u.username,o.* from en_orders o,en_userinfo u where o.userid= " + userid + " and u.id=o.userid and 1 = 1";
...
<head>
    ...
    <script language="javascript">
        function golist(r) {
            window.location = "userorders.jsp?startrow=" + r;
        ...
        function gotosearch() {
            searchForm.action = "userorders.jsp";
            searchForm.submit();
        }

        function jumppage(r, str) {
            window.location = "userorders.jsp?startrow=" + r + str;
...
<form action="userorders.jsp" method="post" name="chuku">
                ...
                <tr bgcolor="#F4F4F4" align="center">
                    <td class="moduleTitle">
                        <font color="#48758C">
                            用户
                            <font color=red>
                                <a href="../member/detail_user.jsp?userid=<%=userid%>" target=_blank>
                                    <%=showname%>
                                </a>
                            </font>
                            订单列表(用户所下所有订单)
                    ...
                    <td>
                        <table width="100%" border="0" cellpadding="2" cellspacing="1">
                            ...
                            <tr bgcolor="#FFFFFF">
                                ...
                                <td align="center"><%=StringUtil.gb2iso4View(order.getProvince())%></td>
                                <td align="center"><%=orderMgr.getSendWay(order.getSendWay())%></td>
                                <td align="center">
                                    <font color="red">
                                        <%if (order.getStatus() == 0) {%>员工作废<%
                                    } else {
        ...
        <td>
            <%
                if (searchflag == 0) {
                    if ((startrow - range) >= 0) {
            %>
            [<a href="userorders.jsp?startrow=<%=startrow-range%>">上一页</a>]
            <%
                }
                if ((startrow + range) < rows) {
            %>
            [<a href="userorders.jsp?startrow=<%=startrow+range%>">下一页</a>]
            <%
                }
            ...
            <%
                ...
                if ((startrow - range) >= 0) {
            %>
            [<a href="userorders.jsp?startrow=<%=startrow-range%><%=jumpstr%>">上一页</a>]
            <%}%>
            <%
                if ((startrow + range) < rows) {
            %>
            [<a href="userorders.jsp?startrow=<%=startrow+range%><%=jumpstr%>">下一页</a>]
            <%
                }
                if (totalpages > 1) {
            %>
...
<center>
    <table width="90%" border="0" cellpadding="0">
        ...
        <tr bgcolor="#d4d4d4" align="right">
            <td bgcolor="#DCE4E7">
                <form name="searchForm" method="post" action="userorders.jsp">
