...
<%
    ...
    if (updateflag == 1) {
        ...
        for (int i = 0; i < ordergo.length; i++) {
            ...
            orderMgr.updateStatus(updateid, 4, authToken.getUserID());
            ...
            scores = a_ord.getOrderscores();
            orderMgr.updateUserScores(userid,scores);
        }
    ...
    if (updateflag == 2) {
        ...
        for (int i = 0; i < ordergo.length; i++) {
            updateid = Long.parseLong(ordergo[i]);
            orderMgr.updateStatus(updateid, 5, authToken.getUserID());
    ...
    if (searchflag == 0) {
        String sql = "select o.* from tbl_orders o where o.siteid = " + siteid + " and o.status=2 and nouse = 1 order by o.createdate desc";
        list = orderMgr.getOrderList(startrow, 0, 2, sql);
        currentlist = orderMgr.getOrderList(startrow, range, 2, sql);
        rows = list.size();
        if (rows < range) {
            totalpages = 1;
            currentpage = 1;
    ...
    if (searchflag != 0) {
        ...
        sqlstr = "select o.* from tbl_orders o where o.status=2 and o.siteid = " + siteid+" and 1 = 1";
    ...
    <script language="javascript">
        ...
        function check(form, tflag) {
            ...
            if (!flag) {
                ...
                var val;
                if (tflag == 1) {
                    val = confirm("将选中定单状态改为已完成？");
                    if (val) {
                        form.action = "receive.jsp?updateflag=1";
                        form.submit();
                    }
                } else if (tflag == 2) {
                    val = confirm("将选中定单状态改为拒收？");
                    if (val) {
                        form.action = "receive.jsp?updateflag=2";
                        form.submit();
                    }
                }
            }
        ...
        function check1(form) {
            ...
            if (!flag) {
                ...
                val = confirm("将选中定单状态改为拒收？");
                if (val == 1) {
                    window.location = "receive.jsp?updateflag=2";
                }
                else
                    window.location = "receive.jsp";
            }
        }
    </script>
</head>
...
<tr bgcolor="#F4F4F4" align="center">
    <td class="moduleTitle"><font color="#48758C">已发货订单列表</font></td>
</tr>
...
<tr bgcolor="#FFFFFF">
    ...
    <td align="center"><%if(payway == null){%>货到付款<%}else{%><%=payway.getCname()==null?"--":StringUtil.gb2iso4View(payway.getCname())%><%}%></td>
...
<input type="button" value="已完成" onclick="javascript:return check(this.form,1);" name="up1">   <input type="button"
                                                                                                      value="拒收"
        ...
        <td>
            <%
                if (searchflag == 0) {
                    if ((startrow - range) >= 0) {
            %>
            [<a href="receive.jsp?startrow=<%=startrow-range%>">上一页</a>]
            <%
                }
                if ((startrow + range) < rows) {
            %>
            [<a href="receive.jsp?startrow=<%=startrow+range%>">下一页</a>]
            <%
                }
            ...
            <%
                ...
                if ((startrow - range) >= 0) {
            %>
            [<a href="receive.jsp?startrow=<%=startrow-range%><%=jumpstr%>">上一页</a>]
            <%}%>
            <%
                if ((startrow + range) < rows) {
            %>
            [<a href="receive.jsp?startrow=<%=startrow+range%><%=jumpstr%>">下一页</a>]
            <%
                }
                if (totalpages > 1) {
            %>
...
<center>
    <table width="90%" border="0" cellpadding="0">
        ...
        <tr bgcolor="#d4d4d4" align="right">
            <td bgcolor="#DCE4E7">
                <form name="searchForm" method="post" action="receive.jsp">
