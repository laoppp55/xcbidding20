...
<%
  ...
  session.setAttribute("backsrc","index-new.jsp");
  ...
  if(searchflag == 0){
    sqlstr = "select * from tbl_bookinfo where showflag=1 and newflag=0  and 1 = 1 order by createdate desc";
  ...
  if(searchflag != 0){
    ...
    int book_type_id = 0;
    if(searchflag != 0){
      sqlstr = "select * from tbl_bookinfo where showflag=1 and newflag=0 and 1 = 1";
...
function gotoSearch(r,p){
  ...
  SearchForm.action = "index-new.jsp?searchflag=1";
  SearchForm.submit();
...
function jumppage(r,str){
  tehuiForm.action = "index-new.jsp?startrow="+r+str;
  tehuiForm.submit();
}
function golistnew(r){
  tehuiForm.action = "index-new.jsp?startrow="+r;
  tehuiForm.submit();
}
</script>
</head>
...
<%
  if(searchflag == 0){
    if((startrow-range)>=0){
%>
[<a href="index-new.jsp?startrow=<%=startrow-range%>">上一页</a>]
<%}%>
<%
  if((startrow+range)<rows){
%>
[<a href="index-new.jsp?startrow=<%=startrow+range%>">下一页</a>]
<%}
 if(totalpages>1){%>
...
<%}
    ...
    if((startrow-range)>=0){
%>
  [<a href="index-new.jsp?startrow=<%=startrow-range%><%=jumpstr%>">上一页</a>]
  <%}
    if((startrow+range)<rows){
  %>
  [<a href="index-new.jsp?startrow=<%=startrow+range%><%=jumpstr%>">下一页</a>]
  <%}
 if(totalpages>1){
                ...
                <tr bgcolor="#FFFFFF">
                  ...
                  <td class="txt">推荐类型：&nbsp;
                    <select name="recommend_flag">
                      <option value=-1>请选择...</option>
                      <option value="0" >不推荐</option>
                      <option value="1">列表推荐</option>
                      <option value="2">新书首页推荐</option>
                    </select>
                  </td>
                </tr>
