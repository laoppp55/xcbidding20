...
<%
  ...
  session.setAttribute("backsrc","index-old.jsp");
...
String sqlstr = "";
  ...
  if(searchflag == 0){
    sqlstr = "select * from tbl_bookinfo where showflag=1 and newflag=2 and 1 = 1 order by createdate desc";
  ...
  if(searchflag != 0){
    ...
    int book_type_id = 0;
    if(searchflag != 0){
      sqlstr = "select * from tbl_bookinfo where showflag=1 and newflag=2 and 1 = 1";
...
function gotoSearch(r,p){
  ...
  SearchForm.action = "index-old.jsp?searchflag=1";
  SearchForm.submit();
...
function jumppage(r,str){
  tehuiForm.action = "index-old.jsp?startrow="+r+str;
  tehuiForm.submit();
}
function golistnew(r){
  tehuiForm.action = "index-old.jsp?startrow="+r;
  tehuiForm.submit();
}
</script>
</head>
  ...
  <table border="0" cellspacing="0" cellpadding="4" bgcolor="#FFFFFF" width="100%">
    ...
    <tr>
     ...
     <select name="recommendflag">
       ...
       <option value="5" >古籍首页详细显示</option>
       <option value="6" >古籍首页列表显示</option>
     </select>
     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<%
  if(searchflag == 0){
    if((startrow-range)>=0){
%>
[<a href="index-old.jsp?startrow=<%=startrow-range%>">上一页</a>]
<%}%>
<%
  if((startrow+range)<rows){
%>
[<a href="index-old.jsp?startrow=<%=startrow+range%>">下一页</a>]
<%}
 if(totalpages>1){%>
...
<%}
    ...
    if((startrow-range)>=0){
%>
  [<a href="index-old.jsp?startrow=<%=startrow-range%><%=jumpstr%>">上一页</a>]
  <%}
    if((startrow+range)<rows){
  %>
  [<a href="index-old.jsp?startrow=<%=startrow+range%><%=jumpstr%>">下一页</a>]
  <%}
 if(totalpages>1){
