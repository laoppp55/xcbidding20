...
<%
    ...
    int tid = ParamUtil.getIntParameter(request, "tid", 0);    
    ...
    */
    ...
    if (doSearch) {
        ...
        try {
            ...
            articleList = orderArticleMgr.getArticlesByPage1(columnID, start, range, flag, username, selectColumns,siteid);
...
function uploadpics()
{
    window.open("../upload/uploadpics.jsp?column=<%=columnID%>", "UploadPics", "width=600,height=400,left=20,top=20,scrollbars");
}
...
<%
    ...
    String[][] operations = {
            ...
            {"退回", "returnarticle.jsp?column=" + columnID},
            {"在审", "auditarticle.jsp?column=" + columnID},
            {"下架", "unusedarticle.jsp?column=" + columnID},
    };
%>
...
<tr class=itm bgcolor="#dddddd" height=20>
    <%if(tid==1){%><td align=center width="4%">添加</td><%}%>
    <td align=center width="4%">RSS</td>
    <td align=center width="4%">状态</td>
    <td align=center width="29%"><a href="articles.jsp?flag=1&column=<%=columnID%>">标题</a></td>
...
<%
  ...
  for (int i = 0; i < articleCount; i++) {
    ...
    if(tid==1){out.println("<input type=radio name=radio value=" + maintitle + " onclick=\"check();\">");
      ...
      if (referID > 0)
        ...
        if(!isown) {
            if(useArticleType == 0)
                ...
                out.println("<td align=center><a href=\"../publish/publish.jsp?column=" + columnID + "&article=" + articleID + "&start=" + start + "&range=" + range + "&source=3&isown="+isown+"\"><img src=../images/button/2scrollbox2.bmp border=0 alt=\"发布文章\"></a></td>");
...
<form action="" method=Post name=searchForm>
    ...
    <div id=searchDiv style="display:none">
        <table border="0" width="98%" cellspacing=0 cellpadding=0 align=center>
            <tr>
                <td width="40%">商品ID：&nbsp;&nbsp;<input name=id size=30></td>
            ...
            <tr>
                <td width="40%">商品状态：<input type=radio name=status value=1>新稿
