...
function golistnew(r){
  xiajiaform.action = "xiajia.jsp?startrow="+r;
  xiajiaform.submit();
}
...
function gotoSearch(r,p){
  ...
  SearchForm.action = "xiajia.jsp?searchflag=1";
  SearchForm.submit();
...
function jumppage(r,str){
  xiajiaform.action = "xiajia.jsp?startrow="+r+str;
  xiajiaform.submit();
}
</script>
</head>
...
<form method="post" action="doxiajia.jsp" name="xiajiaform">
  <table border="0" cellspacing="0" cellpadding="4" bgcolor="#FFFFFF" width="100%">
    <tr>
      <td>
        <table width="100%" border="0" cellpadding="0">
          <tr bgcolor="#F4F4F4" align="center">
            <td class="moduleTitle"><font color="#48758C">图书下架管理</font></td>
          </tr>
              ...
              <table width="100%" border="0" cellpadding="2" cellspacing="1">
                ...
                for(int i=0;i<currentlist.size();i++){
                  ...
                  if(existflag){
                    ...
                    int count = searchMgr.getCount(search2.getBookID());
                    float discprice = search2.getLast_Price() - search2.getSale_Price();
                    if(discprice <= 0)
                    discprice = 0;

                %>
                <tr  bgcolor="#FFFFFF">
                  <td align="center"><input type="checkbox" name="xiajia" value="<%=search2.getBookID()%>"></td>
...
<%
  if(searchflag == 0){
    if((startrow-range)>=0){
%>
[<a href="xiajia.jsp?startrow=<%=startrow-range%>">上一页</a>]
<%}%>
<%
  if((startrow+range)<rows){
%>
[<a href="xiajia.jsp?startrow=<%=startrow+range%>">下一页</a>]
<%}
 if(totalpages>1){%>
...
<%}
    ...
    if((startrow-range)>=0){
%>
  [<a href="xiajia.jsp?startrow=<%=startrow-range%><%=jumpstr%>">上一页</a>]
  <%}
    if((startrow+range)<rows){
  %>
  [<a href="xiajia.jsp?startrow=<%=startrow+range%><%=jumpstr%>">下一页</a>]
  <%}
 if(totalpages>1){
...
<%}
  ...
  <input type="submit" value="下架" onclick="javascript:return check(this.form);">
</form>

<table width="770" border="0" cellspacing="0" cellpadding="4" bgcolor="#FFFFFF">
<form method="post" action="xiajia.jsp" name="SearchForm">
