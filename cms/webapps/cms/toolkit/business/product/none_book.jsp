...
<%
  ...
  if(searchflag == 0){
    list = searchMgr.getAllBook2();
    currentlist = searchMgr.getCurrentAllBook2(startrow,range);
  ...
  if(searchflag != 0){
    ...
    int book_type_id = 0;
    if(searchflag != 0){
      sqlstr = "select * from tbl_bookinfo where 1 = 1";
      ...
      if((gonghuoshangs != "")&&(gonghuoshangs != null)){
       if(!gonghuoshangs.equals("null"))
        sqlstr = sqlstr + " and gonghuoshang like '@"+gonghuoshangs+"@'";
      }
...
<script language="javascript">
  function CheckAll(form){
     for (var i=0;i<form.delbook.length;i++){
        var e = form.delbook[i];
        if (e.name != 'chkAll')
...
function golistnew(r){
  tehuiForm.action = "book.jsp?startrow="+r;
  tehuiForm.submit();
}
...
function gotoSearch(r,p){
  ...
  SearchForm.action = "book.jsp?searchflag=1";
  SearchForm.submit();
...
function jumppage(r,str){
  tehuiForm.action = "book.jsp?startrow="+r+str;
  tehuiForm.submit();
}
...
<form method="post" action="" name="tehuiForm">
      ...
      <td>
        <table width="100%" border="0" cellpadding="0">
          <tr bgcolor="#F4F4F4" align="center">
            <td class="moduleTitle"><font color="#48758C">所有图书列表</font></td>
          </tr>
            ...
            <td>
              <table width="100%" border="0" cellpadding="2" cellspacing="1">
                ...
                for(int i=0;i<currentlist.size();i++){
                  ...
                  int sid = search.getSID();
                  Search search2 = searchMgr.getABatch(sid);

                  bookid = search.getBookID();
                  bookname = search.getBookName();
                  bookname = bookname == null?"":new String(bookname.getBytes("iso8859_1"),"GBK");
                  author = search.getAuthor();
                  author = author == null?"":new String(author.getBytes("iso8859_1"),"GBK");
                  publisher = search.getPublisher();
                  publisher = publisher == null?"":new String(publisher.getBytes("iso8859_1"),"GBK");
                  publishtime = search.getPublishtime();
                  inprice = search.getIn_Price();
                  createdate = search.getCreateDate();

                  int count = searchMgr.getCount(bookid);
                  float discprice = search.getLast_Price() - search.getSale_Price();
                  if(discprice <= 0)
                    discprice = 0;
                %>
                <tr  bgcolor="#FFFFFF" class="txt">
                  <td align="center"><input type="checkbox" name="delbook" value="<%=bookid%>"></td>
                  <td align="center" <%if(search.getHavePic()==1){%>bgcolor="#99CCFF"<%}%>>
                    <input type="radio" value="<%=bookid%>" onclick="javascript:uploadpic(<%=bookid%>);">
                  <td align="center"><%=booktype%></td>
                  <td align="center"><font color="#0000CC"><%=booktypestr%></font></td>
                 <td align="center"><font color="#990099"><%=searchMgr.getBookState(bookid)%></font></td>
                  <td align="center"><%=count%></td>
                  <td align="center"><a href="editbookinfo.jsp?bookid=<%=bookid%>" target=_blank><%=bookname%></a></td>
                  <td align="center"><%=author%></td>
                  <td align="center"><%=publisher%></td>
                  <td align="center"><%=publishtime==null?"":String.valueOf(publishtime).substring(0, 7)%></td>
                  <td align="center"><%=search.getBanCiYinCi()==null?"":new String(search.getBanCiYinCi().getBytes("iso8859_1"),"GBK")%></td>
                  <td align="center"><%=search.getPrinting_Number()%></td>
                  <td align="center"><%=search.getPinXiang()%></td>
                  <td align="center"><%=inprice%></td>
                  <td align="center"><%=search.getLast_Price()%></td>
                  ...
                  <td align="center"><%=search.getRecommend()%></td>
                  <td align="center"><%=search.getStockNum()%></td>
...
<%
  if(searchflag == 0){
    if((startrow-range)>=0){
%>
[<a href="book.jsp?startrow=<%=startrow-range%>">上一页</a>]
<%}%>
<%
  if((startrow+range)<rows){
%>
[<a href="book.jsp?startrow=<%=startrow+range%>">下一页</a>]
<%}
 if(totalpages>1){%>
...
<%}
    ...
    jumpstr=jumpstr+"&havepics="+havepics;

    if((startrow-range)>=0){
%>
  [<a href="book.jsp?startrow=<%=startrow-range%><%=jumpstr%>">上一页</a>]
  <%}
    if((startrow+range)<rows){
  %>
  [<a href="book.jsp?startrow=<%=startrow+range%><%=jumpstr%>">下一页</a>]
  <%}
 if(totalpages>1){
...
<td><img src="images/space.gif" width="1" height="1"></td>
        ...
        <p><input type=button value="搜索" onclick="javascript:return gotoSearch(document.all('publishtime1').value,document.all('publishtime2').value);"><br>
          <br>
        </p>
</td>
</tr>
