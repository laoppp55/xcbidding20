...
<%
    ...
    Tree colTree = TreeManager.getInstance().getProductTree(siteid);
...
<script type="text/javascript">
    function go(columnID)
    {
        ...
        if (isMSIE) {
            ...
            parent.frames['cmsright'].location = "articles.jsp?rightid=<%=rightid%>&tid=<%=tid%>&column=" + columnID;
            return;
        } else {
            ...
            window.parent.frames['cmsright'].location = "articles.jsp?rightid=<%=rightid%>&tid=<%=tid%>&column=" + columnID;
            return;
        }
    }
</script>