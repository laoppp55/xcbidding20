...
<script language="javascript">
  ...
  {
    window.location = "dodeal.jsp?id="+id+"&kind="+kind+"&status="+v+"&userid=<%=userid%>&username=<%=username%>&startrow=<%=startrow%>&range=<%=range%>&backstr=index.jsp";
  }
</script>
</head>
...
<form action="index.jsp" method="post" name="repairform">
      ...
      <td>
        <table width="100%" border="0" cellpadding="0">
          <tr bgcolor="#F4F4F4" align="center">
            <td class="moduleTitle"><font color="#48758C">保修申请</font></td>
          </tr>
          <tr bgcolor="#d4d4d4" align="right">
            <td>
              <table width="100%" border="0" cellpadding="2" cellspacing="1">
                ...
                <tr  bgcolor="#FFFFFF">
                  <td align="center">
                   <a href="index.jsp?userid=<%=appli.getUserID()%>" target=_blank><%=username%></a>
                  </td>
                  <td align="center">
...
<%}
  ...
  <%}
    ...
    if((startrow-range)>=0){
%>
  [<a href="index.jsp?userid=<%=userid%>&username=<%=username%>&startrow=<%=startrow-range%>">上一页</a>]
  <%}%>
  <%
    if((startrow+range)<rows){
  %>
  [<a href="index.jsp?userid=<%=userid%>&username=<%=username%>&startrow=<%=startrow+range%>">下一页</a>]
 <%}
   if(totalpages>1){%>
   &nbsp;&nbsp;第<input type="text" name="jump" value=<%=currentpage%> size="3">页&nbsp;
