...
<script language="javascript">
  function golist(r){
    window.location = "change.jsp?startrow="+r;

  }

  function gotosearch(){
    searchForm.action = "change.jsp";
    searchForm.submit();
  }

  function jumppage(r,str){
    window.location = "change.jsp?startrow="+r+"&"+str;
  }

  function dodeal(id,kind,v)
  {
    window.location = "dodeal.jsp?id="+id+"&kind="+kind+"&status="+v+"&userid=<%=userid%>&username=<%=username%>&startrow=<%=startrow%>&range=<%=range%>&backstr=change.jsp";
  }
</script>
</head>
...
<center>
<form name="searchForm" method="post" action="change.jsp">
...
<form action="change.jsp" method="post" name="repairform">
      ...
      <td>
        <table width="100%" border="0" cellpadding="0">
          <tr bgcolor="#F4F4F4" align="center">
            <td class="moduleTitle"><font color="#48758C">更换申请</font></td>
          </tr>
            ...
            <td>
              <table width="100%" border="0" cellpadding="2" cellspacing="1">
                <tr  bgcolor="#FFFFFF">
                  ...
                  <td align="center" width="48%">详细说明</td>
                ...
                <tr  bgcolor="#FFFFFF">
                  <td align="center">
                   <a href="change.jsp?userid=<%=appli.getUserID()%>" target=_blank><%=username%></a>
                  </td>
                  <td align="center">
                  ...
                  <td align="center"><button style="width:40;height:19;font-size:9pt" onclick="dodeal(<%=appli.getID()%>,<%=kind%>,2)">批准</button></td>
                  <td align="center"><button style="width:40;height:19;font-size:9pt" onclick="dodeal(<%=appli.getID()%>,<%=kind%>,3)">无效</button></td>
                </tr>
                <%}%>
               </table>
            </td>
...
<%
  if(searchflag == 0){
    if((startrow-range)>=0){
%>
[<a href="change.jsp?startrow=<%=startrow-range%>">上一页</a>]
<%}
  if((startrow+range)<rows){
%>
[<a href="change.jsp?startrow=<%=startrow+range%>">下一页</a>]
<%}
  ...
  <%}
    ...
    if((startrow-range)>=0){
%>
  [<a href="change.jsp?userid=<%=userid%>&username=<%=username%>&startrow=<%=startrow-range%>">上一页</a>]
  <%}%>
  <%
    if((startrow+range)<rows){
  %>
  [<a href="change.jsp?userid=<%=userid%>&username=<%=username%>&startrow=<%=startrow+range%>">下一页</a>]
 <%}
   if(totalpages>1){%>
   &nbsp;&nbsp;第<input type="text" name="jump" value=<%=currentpage%> size="3">页&nbsp;
