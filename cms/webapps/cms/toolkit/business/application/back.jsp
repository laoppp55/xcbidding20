...
<script language="javascript">
  function golist(r){
    window.location = "back.jsp?startrow="+r;

  }

  function gotosearch(){
    searchForm.action = "back.jsp";
    searchForm.submit();
  }

  function jumppage(r,str){
    window.location = "back.jsp?startrow="+r+"&"+str;
  }

  function dodeal(id,kind,v)
  {
    window.location = "dodeal.jsp?id="+id+"&kind="+kind+"&status="+v+"&userid=<%=userid%>&username=<%=username%>&startrow=<%=startrow%>&range=<%=range%>&backstr=back.jsp";
  }
</script>
</head>
...
<center>
<form name="searchForm" method="post" action="back.jsp">
...
<form action="back.jsp" method="post" name="repairform">
      ...
      <td>
        <table width="100%" border="0" cellpadding="0">
          <tr bgcolor="#F4F4F4" align="center">
            <td class="moduleTitle"><font color="#48758C">退货申请</font></td>
          </tr>
          <tr bgcolor="#d4d4d4" align="right">
            <td>
              <table width="100%" border="0" cellpadding="2" cellspacing="1">
                ...
                <tr  bgcolor="#FFFFFF">
                  <td align="center">
                   <a href="back.jsp?userid=<%=appli.getUserID()%>" target=_blank><%=username%></a>
                  </td>
                  <td align="center">
...
<%
  if(searchflag == 0){
    if((startrow-range)>=0){
%>
[<a href="back.jsp?startrow=<%=startrow-range%>">上一页</a>]
<%}
  if((startrow+range)<rows){
%>
[<a href="back.jsp?startrow=<%=startrow+range%>">下一页</a>]
<%}
  ...
  <%}
    ...
    if((startrow-range)>=0){
%>
  [<a href="back.jsp?userid=<%=userid%>&username=<%=username%>&startrow=<%=startrow-range%>">上一页</a>]
  <%}%>
  <%
    if((startrow+range)<rows){
  %>
  [<a href="back.jsp?userid=<%=userid%>&username=<%=username%>&startrow=<%=startrow+range%>">下一页</a>]
 <%}
   if(totalpages>1){%>
   &nbsp;&nbsp;第<input type="text" name="jump" value=<%=currentpage%> size="3">页&nbsp;
