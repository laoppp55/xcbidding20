...
<input type="hidden" name="flag" value="1">
      ...
      <td>
        <table width="100%" border="0" cellpadding="0">
          <tr bgcolor="#F4F4F4" align="center">
            <td class="moduleTitle"><font color="#48758C">专题图书管理</font></td>
          </tr>
  ...
  <input type="button" value="实行专题" onclick="javascript:gotodo(1);">
  <input type="button" value="取消专题" onclick="javascript:gotodo(0);">
</form>
...
<%
  if(searchflag == 0){
    if((startrow-range)>=0){
%>
[<a href="zhuanti.jsp?startrow=<%=startrow-range%>">上一页</a>]
<%}%>
<%
  if((startrow+range)<rows){
%>
[<a href="zhuanti.jsp?startrow=<%=startrow+range%>">下一页</a>]
<%}
 if(totalpages>1){%>
...
<%}
    ...
    if((startrow-range)>=0){
%>
  [<a href="zhuanti.jsp?startrow=<%=startrow-range%><%=jumpstr%>">上一页</a>]
  <%}
    if((startrow+range)<rows){
  %>
  [<a href="zhuanti.jsp?startrow=<%=startrow+range%><%=jumpstr%>">下一页</a>]
  <%}
 if(totalpages>1){
