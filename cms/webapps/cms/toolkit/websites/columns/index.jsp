...
<%
    ...
    do {
        ...
        try {
            companyType = websiteManager.getCompanyClass(currentID);
            if (companyType.getParentid() > 0) {
                ...
                buf.append("<a href='javascript:editurlclass(\"").append(ID).append("\")'");
                buf.append("'>");
                ...
                buf.append("<a href='javascript:createnewurlclass(\"").append(ID).append("\")'");
                buf.append("'>");
...
<head>
    ...
    <script language="javascript">
        ...
        function createnewurlclass(troot){
            var winstr = "createcolumn.jsp?parentID=" + troot;
            window.open(winstr,"createnewurlclass","height=350,width=600,scrollbars=yes,titlebar=no,toolbar=no,location=no,menubar=no,directions=no,dependent=no,resizable=yes,channelmode=no");
        }

        function editurlclass(troot){
            var winstr = "editcolumn.jsp?ID=" + troot;
            window.open(winstr,"editurlclass","height=350,width=600,scrollbars=yes,titlebar=no,toolbar=no,location=no,menubar=no,directions=no,dependent=no,resizable=yes,channelmode=no");
        }
    </script>
</head>
...
<center>
    <%
        ...
        operations = new String[][] {
                ...
                {"新建分类信息", "javascript:createnewurlclass('" + treeRoot + "')"}
        };
    %>
    <%@ include file="../inc/titlebar.jsp" %>
    <%
        if (msgno == 0)
            out.println("<span class=cur>网址分类信息创建成功</span>");
        else if (msgno == -1)
            out.println("<span class=cur></span>");
        else
            out.println("<span class=cur>网址分类信息创建失败</span>");
    %>
