...
<%
    ...
    if(success == 1){
        ...
        out.println("alert(\"添加课程成功\");");
        out.println("</script>");
    }
    if(success == 2){
        out.println("<script language=\"javascript\">");
        out.println("alert(\"修改课程成功\");");
        out.println("</script>");
    }
    ...
    currentlist = orderMgr.getTrainClassList(majorcode,projcode,startrow, range, "");
...
<head>
    ...
    <script language="javascript">
        ...
        function Delclass(ID,majorcode,projcode)
        {
            var str = confirm("确认要删除该课程吗？");
            if (str)
                window.location = "deleteClass.jsp?id=" + ID+ "&majorcode="+majorcode+"&projcode="+projcode;
        }
    </script>
</head>
<%
    ...
    String[][] operations = {
            ...
            {"添加新课程", "addclass.jsp?projcode="+projcode+"&majorcode="+majorcode},
            // {"送报员", "index3.jsp"},
...
<table border=1 borderColorDark=#ffffec borderColorLight=#5e5e00 cellPadding=0 cellSpacing=0 width=100%>
    <tr bgcolor="#eeeeee" class=tine>
        <td align=center width="10%">课程名称</td>
        <td align=center width="5%">课程编码</td>
    ...
    <tr bgcolor="#ffffff" class=line>
        <td align=center><%=training.getClassname()==null?"":training.getClassname()%></td>
        <td align=center><%=training.getClasscode()==null?"":training.getClasscode()%></td>
        <td align=center><%=training.getPrice()%></td>
        <td align=center><%=training.getCreatedate()%></td>
        <%--<td align=center><a href="Trainmajorclass.jsp?majorcode=<%=majorcode%>">管理课程</a></td>--%>
        <td align=center><a href="updateTrainclass.jsp?id=<%=id%>&majorcode=<%=majorcode%>&projcode=<%=projcode%>">修改</a></td>
        <td align=center><a href="javascript:Delclass('<%=id%>','<%=majorcode%>','<%=projcode%>')">删除</a></td>
    </tr>
