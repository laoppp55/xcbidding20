...
<center>
    <%
        ...
        operations = new String[][] {
                {"新建分类", "addcompanyclass.jsp?parentID=" + treeRoot}
        };
    %>
    <%@ include file="../../inc/titlebar.jsp" %>
    <%
        if (msgno == 0)
            out.println("<span class=cur>分类信息创建成功</span>");
        else if (msgno == -1)
            out.println("<span class=cur></span>");
        else
            out.println("<span class=cur>分类信息创建失败</span>");
    %>
    <table border=1 borderColorDark="#ffffec" borderColorLight="#5e5e00" cellPadding=0 cellSpacing=0 width='100%'>
        <tr class=tine bgcolor="#dddddd" height=20>
            <td width='52%'>&nbsp;<%=TREE_ROOT%>&nbsp;<%=treeNodes[0].getEnName()%></td>
            <td align=center width='16%'>分类名称</td>
