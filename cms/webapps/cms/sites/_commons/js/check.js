...
function onReadyStateChange(){
    ...
    if (ready ==4) {
        ...
        if (posi > -1) {
           data = data.substring(posi+1);
           posi = data.indexOf("}");
           data = data.substring(0,posi);
        }

        if (data == "nologin") {
            alert("用户名或密码不正确，请重新登录。");
        } else if (data == "ug-null") {
            ;
        } else {
...
/*function check(form) {
    var params = "name=" + form.username.value + "&pass=" + form.password.value;
    sendRequest(params);
}
...
function checklogin(markid) {
    ...
    if (req) {
        req.onreadystatechange=onReadyStateChange;
        req.open("POST", "/_commons/checklogin.jsp?markid="+markid);
        req.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
        req.send(null);
    }
}
