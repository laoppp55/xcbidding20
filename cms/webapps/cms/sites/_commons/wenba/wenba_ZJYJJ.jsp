...
User user = (User)session.getAttribute("user");
	...
	String cid = session.getAttribute("zjyjjcid")==null?"0":(String)session.getAttribute("zjyjjcid");
	session.setAttribute("zjyjjcid",null);
	...
	String thispage = (String)session.getAttribute("YJJthispage")==null?"0":(String)session.getAttribute("YJJthispage");
	String allpages = (String)session.getAttribute("YJJallpages")==null?"0":(String)session.getAttribute("YJJallpages");
	String perpagenum = session.getAttribute("YJJpagenum")==null?"0":(String)session.getAttribute("YJJpagenum");
	List pagelist = (List)session.getAttribute("YJJpagelist");
	if(pagelist==null){
		response.sendRedirect("doyijiejue.jsp");
	}
...
function gonum(){
    	...
    	if(gopagenum==""){
    		...
    		document.form1.action = "doyijiejue.jsp?province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>";
...
function sousuo(){
	...
	document.form1.action = "doyijiejue.jsp?fenlei="+fenlei+"&province="+province+"&keyword="+keyword;
    document.form1.submit();
}
</script>
<body>
                ...
                <tr>
                  <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      ...
                      <td width="248" align="center" background="/images/wenba_woyaoda_r7_c7.jpg" class="blackc12c">已解决的问题</td>
                      <td align="center"><img src="/images/bai.gif" width="1" height="1" /></td>
                ...
                <tr>
                  <td valign="top" background="/images/wenba_woyaoda_r8_c5.jpg"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
						...
						%>
                		<a href="doyijiejue.jsp?thispage=1&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">首页</a>&nbsp;&nbsp;&nbsp;<a href="doyijiejue.jsp?thispage=<%=Integer.parseInt(thispage)-1 %>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">上一页</a>
                		<%
							}
							if (Integer.parseInt(allpages) > Integer.parseInt(thispage)){
							%>
              				&nbsp;&nbsp;<a href="doyijiejue.jsp?thispage=<%=Integer.parseInt(thispage)+1 %>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">下一页</a>&nbsp;&nbsp; &nbsp;<a href="doyijiejue.jsp?thispage=<%=Integer.parseInt(allpages)%>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">末页</a>
              				<%
							}
							%>
