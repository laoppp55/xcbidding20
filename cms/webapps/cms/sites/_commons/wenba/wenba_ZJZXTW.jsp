...
User user = (User)session.getAttribute("user");
	...
	String cid = session.getAttribute("zjzxtwcid")==null?"0":(String)session.getAttribute("zjzxtwcid");
	session.setAttribute("zjzxtwcid",null);
	...
	String thispage = (String)session.getAttribute("ZXTWthispage")==null?"0":(String)session.getAttribute("ZXTWthispage");
	String allpages = (String)session.getAttribute("ZXTWallpages")==null?"0":(String)session.getAttribute("ZXTWallpages");
	String perpagenum = session.getAttribute("ZXTWpagenum")==null?"0":(String)session.getAttribute("ZXTWpagenum");
	List pagelist = (List)session.getAttribute("ZXTWpagelist");
	if(pagelist==null){
		response.sendRedirect("dozuixintiwen.jsp");
	}
...
function gonum(){
    	...
    	if(gopagenum==""){
    		...
    		document.form1.action = "dozuixintiwen.jsp?province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>";
...
function sousuo(){
	...
	document.form1.action = "dozuixintiwen.jsp?fenlei="+fenlei+"&province="+province+"&keyword="+keyword;
    document.form1.submit();
}
</script>
<body>
            ...
            <tr>
              ...
              <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td width="248" height="30" align="center" background="/images/wenba_woyaoda_r7_c5.jpg" class="blackc12c">最新提出问题</td>
                      ...
                      <td align="center"><img src="/images/bai.gif" width="1" height="1" /></td>
                      <td width="247" align="center" background="/images/wenba_woyaoda_r7_c10.jpg" class="blackc12c"><a href="dolinghuida.jsp">零回答的问题</a></td>
                    </tr>
                  </table></td>
                </tr>
                <tr>
                  <td valign="top" background="/images/wenba_woyaoda_r8_c5.jpg"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
						...
						%>
                		<a href="dozuixintiwen.jsp?thispage=1&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">首页</a>&nbsp;&nbsp;&nbsp;<a href="dozuixintiwen.jsp?thispage=<%=Integer.parseInt(thispage)-1 %>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">上一页</a>
                		<%
							}
							if (Integer.parseInt(allpages) > Integer.parseInt(thispage)){
							%>
              				&nbsp;&nbsp;<a href="dozuixintiwen.jsp?thispage=<%=Integer.parseInt(thispage)+1 %>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">下一页</a>&nbsp;&nbsp; &nbsp;<a href="dozuixintiwen.jsp?thispage=<%=Integer.parseInt(allpages)%>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">末页</a>
              				<%
							}
							%>
