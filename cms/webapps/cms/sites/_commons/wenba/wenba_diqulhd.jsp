...
<%
	...
	String thispage = (String)session.getAttribute("difanglinghdthispage")==null?"0":(String)session.getAttribute("difanglinghdthispage");
	String allpages = (String)session.getAttribute("difanglinghdallpages")==null?"0":(String)session.getAttribute("difanglinghdallpages");
	String perpagenum = session.getAttribute("difanglinghdpagenum")==null?"0":(String)session.getAttribute("difanglinghdpagenum");
	List pagelist = (List)session.getAttribute("difanglinghdpagelist");
	if(pagelist==null){
		response.sendRedirect("dodifangZXTW.jsp");
	}
	String str3 = (String)session.getAttribute("str3");
	session.setAttribute("str3",null);
	session.setAttribute("difanglinghdthispage",null);
	session.setAttribute("difanglinghdallpages",null);
	session.setAttribute("difanglinghdpagenum",null);
	session.setAttribute("difanglinghdpagelist",null);
	String provincename = "";
	String columnid = "0";
	String keyword = "";
	if(str3!=null){
...
function gonum(){
    	...
    	if(gopagenum==""){
    		...
    		document.form1.action = "dodifangLHD.jsp?proname=<%=provincename %>&cid=<%=columnid %>&keys=<%=keyword %>";
...
function sousuo_fenlei(){
	...
	var url ="dodifangLHD.jsp?proname=<%= provincename%>&cid=" +fenleiID +"&keys=" + keys;
	if(fenleiID==""){
		alert("请选择分类");
		return ;
	}
                    ...
                    <tr>
                      <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            ...
                            <td width="248" align="center" background="/images/wenba_woyaoda_r7_c7.jpg" class="blackc12c"><a href="dodifangYJJ.jsp?proname=<%=provincename %>">已解决的问题</a></td>
                            <td align="center"><img src="/images/bai.gif" width="1" height="1" /></td>
                            <td width="247" align="center" background="/images/wenba_woyaoda_r7_c5.jpg" class="blackc12c"><a href="dodifangLHD.jsp?proname=<%=provincename %>" >零回答的问题</a></td>
                          </tr>
                      </table></td>
                    </tr>
                    <tr>
                      <td valign="top" background="/images/wenba_woyaoda_r8_c5.jpg"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
						...
						%>
                		<a href="dodifangYJJ.jsp?thispage=1">首页</a>&nbsp;&nbsp;&nbsp;<a href="dodifangYJJ.jsp?thispage=<%=Integer.parseInt(thispage)-1 %>">上一页</a>
                		<%
							}
							if (Integer.parseInt(allpages) > Integer.parseInt(thispage)){
							%>
              				&nbsp;&nbsp;<a href="dodifangYJJ.jsp?thispage=<%=Integer.parseInt(thispage)+1 %>">下一页</a>&nbsp;&nbsp; &nbsp;<a href="dodifangYJJ.jsp?thispage=<%=Integer.parseInt(allpages)%>">末页</a>
              				<%
							}
							%>
