...
%>
	<% //0回答问题分页
		...
		int intPageSize_0ans=10; //每页显示的记录数
	    ...
	    List pagelist_0ans = iwenba.get0ansQuestions_num(classid);
...
function sousuo_fenlei(){
	...
	var url ="wenba_leibiao0a.jsp?pro=<%= Pro%>&cid=" +fenleiID +"&keys=" + keys;
	if(fenleiID==""){
		alert("请选择分类");
		return ;
	}
            ...
            <tr>
              ...
              <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td width="248" height="30" align="center" background="/images/wenba_woyaoda_r7_c10.jpg" class="blackc12c"><a href="#" onclick="wenti_change()">最新提出问题</a></td>
                      ...
                      <td align="center"><img src="/images/bai.gif" width="1" height="1" /></td>
                      <td width="247" align="center" background="/images/wenba_woyaoda_r7_c5.jpg" class="blackc12c"><a href="#" onclick="wenti_ans_change()">零回答的问题</a></td>
                    </tr>
                  </table></td>
                </tr>
                <tr>
                    ...
                    <tr>
                      <td valign="top"><table width="100%" border="0" cellspacing="2" cellpadding="0">
                        ...
                        <%	
							...
							String color = "";
                    		List wenti_0ans = iwenba.get0ansQuestions(end_no_0ans,begin_no_0ans,classid);
                    		wenti wta = null;
                    		String date_str_ans = null;
                    		for (int i = 0; i < wenti_0ans.size(); i++) {
                    			if(i%2==0){
									color = color2;
								}else{
									color = color1;
								}
                    			wta = (wenti) wenti_0ans.get(i);
                    			date_str_ans = wta.getCreatedate().toString();
                    			int posi = date_str_ans.indexOf(" ");
                    			date_str_ans = date_str_ans.substring(0, posi);
                    			String titles = wta.getTitles().trim();  
			                    String str;
			                    if(titles.length()>18) {
                    	...
                    	%>
                        <tr>
                          <td height="25" align="left" bgcolor="<%= color%>">&nbsp;&nbsp;[<%= wta.getCname()%>]</td>
                          <td align="left" bgcolor="<%= color%>">&nbsp;&nbsp;
                        	...
                        	<a href="javascript:xiazaij()">下载</a>
                        	<%} %>
                          </td>
                          <script type="text/JavaScript">
                      	function xiazaij(){
                          ...
                          <td align="center" bgcolor="<%= color%>"><%= wta.getProvince()%></td>
                          <td align="center" bgcolor="<%= color%>"><%= wta.getXuanshang()%>分</td>
                          <td align="center" bgcolor="<%= color%>"><%= wta.getAnwsernum()%></td>
                          <td align="center" bgcolor="<%= color%>">[<%= date_str_ans%>]</td>
                        </tr>
                        <%} %>
						<tr>
                    ...
                    <tr>
                      <td height="25" align="right" valign="bottom">
					  		<%if(intPageCount_0ans>1) {%>
					  			<%if (intPage_0ans > 1) {%>
	                    		<a href="wenba_third_fenlei0ans.jsp?page_0ans=<%=intPage_0ans - 1%>&cid=<%=classid%>"><img src="/images/wenba_woyaoda_r9_c12.jpg" width="4" height="7" border="0" align="absmiddle" /></a>
	                    		<%}%>&nbsp;
	                    		<!-- 共<%= intPageCount_0ans%>页 -->
	                    		<a href="wenba_third_fenlei0ans.jsp?page_0ans=1&cid=<%=classid%>">首页</a>
	                    		<%if (intPage_0ans > 1) {%>
	                    		<a href="wenba_third_fenlei0ans.jsp?page_0ans=<%=intPage_0ans - 1%>&cid=<%=classid%>">上一页</a><%}%>
	                    		<%if (intPage_0ans < intPageCount_0ans) {%>
	                    		<a href="wenba_third_fenlei0ans.jsp?page_0ans=<%=intPage_0ans + 1%>&cid=<%=classid%>">下一页</a><%}%>
	                            <a href="wenba_third_fenlei0ans.jsp?page_0ans=<%=intPageCount_0ans%>&cid=<%=classid%>">尾页</a>	 &nbsp;
								<%if (intPage_0ans < intPageCount_0ans) {%>
	                    		<a href="wenba_third_fenlei0ans.jsp?page_0ans=<%=intPage_0ans + 1%>&cid=<%=classid%>"><img src="/images/wenba_woyaoda_r9_c14.jpg" width="4" height="7" border="0" align="absmiddle" /></a>
	                    		<%}%>&nbsp;&nbsp;</td>
	                    	<%} %>
                    </tr>
                  </table>
