...
User user = (User)session.getAttribute("user");
	...
	String cid = session.getAttribute("yjjcid")==null?"0":(String)session.getAttribute("yjjcid");
	session.setAttribute("yjjcid",null);
   ...
   // List top10_wenti_0anwser = iwenba.getTop10Questions0Answer(classid); 
	...
	String thispage = (String)session.getAttribute("yijjthispage")==null?"0":(String)session.getAttribute("yijjthispage");
	String allpages = (String)session.getAttribute("yijjallpages")==null?"0":(String)session.getAttribute("yijjallpages");
	String perpagenum = session.getAttribute("yijjpagenum")==null?"0":(String)session.getAttribute("yijjpagenum");
	List pagelist = (List)session.getAttribute("yijjpagelist");
	if(pagelist==null){
		response.sendRedirect("doYJJ.jsp");
	}
...
function gonum(){
    	...
    	if(gopagenum==""){
    		...
    		document.form1.action = "doYJJ.jsp?province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>";
...
function sousuo(){
	...
	document.form1.action = "doYJJ.jsp?fenlei="+fenlei+"&province="+province+"&keyword="+keyword;
    document.form1.submit();
}
</script>
<body>
            ...
            <tr>
              ...
              <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td width="248" height="30" align="center" background="/images/wenba_woyaoda_r7_c7.jpg" class="blackc12c"><a href="doZXTW.jsp">最新提出问题</a></td>
                      <td align="center"><img src="/images/bai.gif" width="1" height="1" /></td>
                      <td width="248" align="center" background="/images/wenba_woyaoda_r7_c5.jpg" class="blackc12c"><a href="doYJJ.jsp">已解决的问题</a></td>
                      <td align="center"><img src="/images/bai.gif" width="1" height="1" /></td>
                ...
                <tr>
                  <td height="550" valign="top" background="/images/wenba_woyaoda_r8_c5.jpg"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
						...
						%>
                		<a href="doYJJ.jsp?thispage=1&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">首页</a>&nbsp;&nbsp;&nbsp;<a href="doYJJ.jsp?thispage=<%=Integer.parseInt(thispage)-1 %>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">上一页</a>
                		<%
							}
							if (Integer.parseInt(allpages) > Integer.parseInt(thispage)){
							%>
              				&nbsp;&nbsp;<a href="doYJJ.jsp?thispage=<%=Integer.parseInt(thispage)+1 %>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">下一页</a>&nbsp;&nbsp; &nbsp;<a href="doYJJ.jsp?thispage=<%=Integer.parseInt(allpages)%>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">末页</a>
              				<%
							}
							%>
