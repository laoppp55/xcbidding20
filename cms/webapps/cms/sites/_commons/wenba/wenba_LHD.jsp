...
User user = (User)session.getAttribute("user");
	...
	String cid = session.getAttribute("lhdcid")==null?"0":(String)session.getAttribute("lhdcid");
	session.setAttribute("lhdcid",null);
   ...
   // List top10_wenti_0anwser = iwenba.getTop10Questions0Answer(classid); 
	...
	String thispage = (String)session.getAttribute("linghdthispage")==null?"0":(String)session.getAttribute("linghdthispage");
	String allpages = (String)session.getAttribute("linghdallpages")==null?"0":(String)session.getAttribute("linghdallpages");
	String perpagenum = session.getAttribute("linghdpagenum")==null?"0":(String)session.getAttribute("linghdpagenum");
	List pagelist = (List)session.getAttribute("linghdpagelist");
...
function gonum(){
    	...
    	if(gopagenum==""){
    		...
    		document.form1.action = "doLHD.jsp?province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>";
...
function sousuo(){
	...
	document.form1.action = "doLHD.jsp?fenlei="+fenlei+"&province="+province+"&keyword="+keyword;
    document.form1.submit();
}
</script>
<body>
            ...
            <tr>
              ...
              <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td width="248" height="30" align="center" background="/images/wenba_woyaoda_r7_c10.jpg" class="blackc12c"><a href="doZXTW.jsp">最新提出问题</a></td>
                      ...
                      <td align="center"><img src="/images/bai.gif" width="1" height="1" /></td>
                      <td width="247" align="center" background="/images/wenba_woyaoda_r7_c5.jpg" class="blackc12c"><a href="doLHD.jsp">零回答的问题</a></td>
                    </tr>
                  </table></td>
                </tr>
                <tr>
                  <td height="550" valign="top" background="/images/wenba_woyaoda_r8_c5.jpg"><table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
						...
						%>
                		<a href="doLHD.jsp?thispage=1&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">首页</a>&nbsp;&nbsp;&nbsp;<a href="doLHD.jsp?thispage=<%=Integer.parseInt(thispage)-1 %>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">上一页</a>
                		<%
							}
							if (Integer.parseInt(allpages) > Integer.parseInt(thispage)){
							%>
              				&nbsp;&nbsp;<a href="doLHD.jsp?thispage=<%=Integer.parseInt(thispage)+1 %>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">下一页</a>&nbsp;&nbsp; &nbsp;<a href="doLHD.jsp?thispage=<%=Integer.parseInt(allpages)%>&province=<%=provincename %>&fenlei=<%=columnid %>&keyword=<%=keyword %>">末页</a>
              				<%
							}
							%>
